  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
        <div>
          <h3 className="text-3xl font-black text-white tracking-tight">Sales Analytics</h3>
        ...
        <div className="flex flex-wrap gap-3">
          <button 
            onClick={handlePrint}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 px-5 py-2.5 rounded-xl text-xs font-bold transition-all border border-slate-700"
          >
      ...
      {/* Filter Bar */}
      <div className="bg-[#1e293b] p-6 rounded-[2rem] border border-slate-700/50 shadow-xl flex flex-wrap items-end gap-6 no-print">
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="bg-slate-900 border border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white focus:border-indigo-500 outline-none w-48"
            />
          </div>
        </div>
        <div className="space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button 
            ...
            className="bg-slate-800 hover:bg-slate-700 text-slate-400 p-3 rounded-xl transition-all border border-slate-700"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#1e293b] p-8 rounded-[2.5rem] border border-slate-700/50 shadow-xl relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Total Orders</p>
            <h4 className="text-4xl font-black text-white tracking-tighter">{reportData?.summary?.totalOrders || 0}</h4>
          ...
          <ShoppingCart className="absolute -right-6 -bottom-6 w-32 h-32 opacity-[0.03] group-hover:scale-110 transition-transform duration-500 text-white" />
        </div>

        <div className="bg-[#1e293b] p-8 rounded-[2.5rem] border border-slate-700/50 shadow-xl relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Products Sold</p>
            <h4 className="text-4xl font-black text-white tracking-tighter">{reportData?.summary?.totalProducts || 0}</h4>
            <div className="mt-6 flex items-center gap-2 text-[10px] font-bold text-blue-400 bg-blue-400/10 w-fit px-3 py-1.5 rounded-full">
              <Package className="w-3 h-3" />
              Unique Items Sold
            </div>
          </div>
          <Package className="absolute -right-6 -bottom-6 w-32 h-32 opacity-[0.03] group-hover:scale-110 transition-transform duration-500 text-white" />
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {reportData?.salesByDate?.length === 0 ? (
                  ...
                  <tr key={item._id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="p-5 text-sm font-bold text-slate-300">{item._id}</td>
                    <td className="p-5">
                      <span className="bg-slate-800 text-slate-400 px-2.5 py-1 rounded-lg text-[10px] font-black">{item.orders}</span>
                    </td>
                    <td className="p-5 text-sm font-black text-white text-right">₹{item.revenue.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {reportData?.salesByProduct?.length === 0 ? (
                  ...
                  <tr key={item._id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="p-5">
                      <p className="text-sm font-bold text-slate-300 line-clamp-1">{item.name}</p>
